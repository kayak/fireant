...
from fireant import DataSet, Field, DataType
from fireant.dataset.filters import ComparisonOperator
from fireant.dataset.references import ReferenceFilter, DayOverDay
...
class ReferenceFilterTests(TestCase):
    ...
    def test_reference_filter_with_greater_than(self):
        reference_filter = ReferenceFilter(
            self.dataset.fields.metric0, ComparisonOperator.gt, 5
        )
    ...
    def test_reference_filter_with_greater_than_or_equal(self):
        reference_filter = ReferenceFilter(
            self.dataset.fields.metric0, ComparisonOperator.gte, 5
        )
        reference = DayOverDay(
            self.dataset.fields.timestamp, filters=[reference_filter]
        )
    ...
    def test_reference_filter_with_less_than(self):
        reference_filter = ReferenceFilter(
            self.dataset.fields.metric0, ComparisonOperator.lt, 5
        )
        reference = DayOverDay(
            self.dataset.fields.timestamp, filters=[reference_filter]
        )
    ...
    def test_reference_filter_with_less_than_or_equal(self):
        reference_filter = ReferenceFilter(
            self.dataset.fields.metric0, ComparisonOperator.lte, 5
        )
        reference = DayOverDay(
            self.dataset.fields.timestamp, filters=[reference_filter]
        )
    ...
    def test_reference_filter_with_equal_to(self):
        reference_filter = ReferenceFilter(
            self.dataset.fields.metric0, ComparisonOperator.eq, 5
        )
        reference = DayOverDay(
            self.dataset.fields.timestamp, filters=[reference_filter]
        )
    ...
    def test_reference_filter_with_not_equal_to(self):
        reference_filter = ReferenceFilter(
            self.dataset.fields.metric0, ComparisonOperator.ne, 5
        )
        reference = DayOverDay(
            self.dataset.fields.timestamp, filters=[reference_filter]
        )
    ...
    def test_reference_filter_with_2_filters_combined(self):
        reference_filter_gt = ReferenceFilter(
            self.dataset.fields.metric0, ComparisonOperator.gt, 3
        )
        reference_filter_lt = ReferenceFilter(
            self.dataset.fields.metric0, ComparisonOperator.lt, 10
        )
        reference = DayOverDay(
